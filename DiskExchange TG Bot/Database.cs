...
namespace DiskExchange_TG_Bot
{
    ...
    class Database
    {
        ...
        public Database()                                                                                                                                                                                                                                    
        {
            ...
            string path2 = @"URI=file:C:\Users\King\Desktop\DiskExchangeDB.db";
        ...
        {
            //cmd = new SQLiteCommand($"SELECT * FROM fullOffers WHERE name LIKE '%{query}%'", connection);
            //rdr = cmd.ExecuteReader();
            ...
            //}
            //return ret;
            cmd = new SQLiteCommand($"SELECT * FROM fullOffers", connection);
            rdr = cmd.ExecuteReader();
            discsArray[] ret = new discsArray[0];
            while (rdr.Read())
            {
            ...
            ret = ret.Where(q => ret.All(k => q.name.ToLower().Contains(query))).ToArray();
            return ret;
        }

        internal discsArray[] GetDiscsArrays() // Done
        {
            cmd = new SQLiteCommand($"SELECT * FROM fullOffers", connection);
            rdr = cmd.ExecuteReader();
            discsArray[] ret = new discsArray[0];
        ...
        public string GetUserPhoneFirstCheck(int userId) // Done
        {
            cmd = new SQLiteCommand($"SELECT phone FROM users_info WHERE id = {userId}", connection);
            rdr = cmd.ExecuteReader();
            rdr.ReadAsync();
            string str = rdr.GetString(0);
            return str;
        ...
        internal void SetCondition(string n, int offerId, bool getIdFromUser = false)
        {
            if (getIdFromUser)
                offerId = GetEditOfferId(offerId);
            cmd = new SQLiteCommand($"UPDATE offers SET condition = '{n}' WHERE id = {offerId}", connection);
            cmd.ExecuteNonQueryAsync();
        }
